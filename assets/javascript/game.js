$(document).ready(function() {
  ...
  function buildChar(charID) {
    ...
    resetGame("#enemiesSelect", players);
  ...
  function resetGame(target) {
    ...
    $(target)
      .children()
      .remove();
    console.log("running resetGame loop for " + players.length + " times.");
    for (var i = 0; i < players.length; i++) {
      if (players[i].playAvail) {
        console.log("creating div for ", target);
        $(target).append("<div />");
